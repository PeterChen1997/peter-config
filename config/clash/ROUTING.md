...
4. `peter-ai` + `bm7-openai/claude/gemini` → `AI 专线`
   （**必须在媒体规则之前**：Gemini 等 Google 系域名会被 GlobalMedia 误伤）
5. `bm7-youtube/netflix/tiktok/spotify/twitter/instagram/telegram/globalmedia/speedtest` → `大流量`
6. `peter-heavy` → `大流量`（自维护流量大户：容器镜像、HuggingFace 等）
7. `peter-proxy` → `AI 专线`（强制代理杂项：开发/设计/支付工具）
